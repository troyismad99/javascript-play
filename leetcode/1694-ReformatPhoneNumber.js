...
function reformatNumber(number) {
    ...
    // remove the extra characters '-' and ' '
    ...
    while (i < number.length) {
        ...
        // are we at the last two digits?
        if (number.length - i == 2){
            newNumber.push(number[i++]);
            newNumber.push(number[i++]);
        }
        // are there only 4 digits left?
        ...
        // not last 2 or 4, so grab next three digits
        else if (number.length - i >= 3) {
            newNumber.push(number.slice(i, i+3));
            i += 3;

            // add dash if we are not at the end
            if ( i != number.length) {
                newNumber.push("-");
            }
        }
...
var WayneGretzky = "99";
console.log(reformatNumber(WayneGretzky));